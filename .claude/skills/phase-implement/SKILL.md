...
description: Use when the user wants to implement a phase from docs/class-system-detailed-plan.md.
             Triggers on: "fase", "phase", "implementer fase", "neste fase", "phase_implement",
             "fortsett implementering", "kjør fase". Orchestrates step-by-step implementation
             of one phase at a time with verification, reporting, and plan updates.
             Activate when user says "/phase_implement" or asks to implement a phase.
---
...
3. Sjekk relevant eksisterende kode: finn filene som skal berøres av fasen (via Grep / Glob).
...
- Kjør `npm run build` i prosjektrot for å bekrefte at kodebasen er grønn FØR endringer.
...
- Gjenbruk eksisterende UI-primitiver (`FantasyButton`, `FantasyPanel`, `FantasyProgressBar`, `FantasyIcon`, `PerkCard` osv.).

---

## Steg 4: Post-Fase Verifisering

1. **Build-check:** Kjør `npm run build`. Null TypeScript-feil er kravet.
...
| **Aldri skriv kode over røde builds** | Pre-build MÅ være grønn |
